...
import { confirmNaver } from '@/lib/payments';
...
export async function POST(request: Request) {
  ...
  if (idempotencyKey) {
    const cached = await getIdempotentResult<{ success: boolean; orderNumber: string }>(
      `naver:${session.user.id}:${idempotencyKey}`,
    );
  ...
  try {
    const confirmed = await confirmNaver({
    ...
    if (idempotencyKey) {
      await setIdempotentResult(`naver:${session.user.id}:${idempotencyKey}`, response);
    }
    return NextResponse.json(response);
  } catch (err) {
    const message = err instanceof Error ? err.message : '결제 처리에 실패했습니다.';
    logger.warn('payment.naver_failed', { orderId: order.id, error: message });
